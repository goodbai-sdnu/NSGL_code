...
%   distances.  Rows of X correspond to points（X的行表示数据点）, columns correspond to
%   variables（X的列表示变量）.  KMEANS returns an N-by-1 vector label containing the
%   cluster indices of each point.%%%返回一个n行1列的标签向量，表示出每个点的集群指数。
%
%   [label, center] = LITEKMEANS(X, K) returns the K cluster centroid
%   locations in the K-by-P matrix center.%%返回聚类中心的位置
%
%   [label, center, bCon] = LITEKMEANS(X, K) returns the bool value bCon to
%   indicate whether the iteration is converged.  %%%返回布尔值bCon来表示是否收敛。
%
%   [label, center, bCon, SUMD] = LITEKMEANS(X, K) returns the
%   within-cluster sums of point-to-centroid distances in the 1-by-K vector
%   sumD. 用一个1*k的向量返回类内点到中心距离的和。  
%
%   [label, center, bCon, SUMD, D] = LITEKMEANS(X, K) returns
%   distances from each point to every centroid in the N-by-K matrix
%   D.用n*k的矩阵返回每个点到中心的距离。
%
